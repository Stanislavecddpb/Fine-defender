...
Спецификации: аддендум по личному кабинету (авторизация, мультитенантность) —
[`docs/TZ_addendum_cabinet.md`](docs/TZ_addendum_cabinet.md).
...
**Личный кабинет** ([src/fine_defender/static/dashboard.html](src/fine_defender/static/dashboard.html))
отдаётся по `/`: вход/регистрация, ввод WB-токена, кнопка «Запустить проверку»,
список штрафов с подсветкой дедлайнов, сводка «отбито ₽», карточка штрафа, кнопки
«сгенерировать претензию» и «отметить возврат». Данные **изолированы по тенанту**:
`seller_id` берётся из сессии, доступ к чужому объекту → 404 (см.
[docs/TZ_addendum_cabinet.md](docs/TZ_addendum_cabinet.md)).

Самый простой способ проверить продукт **без реального кабинета WB**: поднять стек
(`docker compose up`), открыть `http://localhost:8000/` и войти под демо-доступом
**`demo@fine-defender.ru` / `demo12345`** (или зарегистрировать нового селлера и
нажать «Запустить проверку» — данные подтянутся из демо-фикстуры, `WB_CLIENT_MODE=mock`).
